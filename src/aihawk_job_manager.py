...
class AIHawkJobManager:
    ...
    def is_blacklisted(self, job_title, company, link, job_location):
        ...
        title_blacklisted = any(
            word in job_title_words for word in map(str.lower, self.title_blacklist)
        )
        company_blacklisted = company.strip().lower() in (
            word.strip().lower() for word in self.company_blacklist
        )
        location_blacklisted = job_location.strip().lower() in (
            word.strip().lower() for word in self.location_blacklist
        )
        link_seen = link in self.seen_jobs
        is_blacklisted = (
            title_blacklisted
            or company_blacklisted
            or location_blacklisted
            or link_seen
        )
