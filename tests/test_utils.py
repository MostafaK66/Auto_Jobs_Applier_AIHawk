...
from src.utils import (chrome_browser_options, ensure_chrome_profile,
                       is_scrollable, printred, printyellow, scroll_slow)
...
def test_is_scrollable(mocker):
    ...
    mock_element.get_attribute.side_effect = (
        lambda attr: "1000" if attr == "scrollHeight" else "500"
    )
...
def test_scroll_slow(mocker):
    ...
    mock_element.get_attribute.side_effect = (
        lambda attr: "2000" if attr == "scrollHeight" else "0"
    )
...
def test_scroll_slow_element_not_scrollable(mocker):
    ...
    mock_element.get_attribute.side_effect = (
        lambda attr: "1000" if attr == "scrollHeight" else "1000"
    )
    mock_element.is_displayed.return_value = True
