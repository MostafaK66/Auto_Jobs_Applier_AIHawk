...
def next_job_page(self, position, location, job_page):
    logger.debug(
        f"Navigating to next job page: {position} in {location}, page {job_page}"
    )
    self.driver.get(
...
def test_get_jobs_from_page_no_jobs(mocker, job_manager):
    ...
    mocker.patch.object(
        job_manager.driver, "find_element", side_effect=NoSuchElementException
    )
...
def test_get_jobs_from_page_with_jobs(mocker, job_manager):
    ...
    mocker.patch.object(
        job_manager.driver, "find_element", return_value=mock_no_jobs_element
    )

    # Mock the page_source
    mocker.patch.object(
        job_manager.driver, "page_source", return_value="some page content"
    )
...
def test_apply_jobs_with_jobs(mocker, job_manager):
    ...
    no_jobs_element.text = (
        ""  # Empty text means "No matching jobs found" is not present
    )
    mocker.patch.object(
        job_manager.driver, "find_element", return_value=no_jobs_element
    )

    # Mock the page_source to simulate what the page looks like when jobs are present
    mocker.patch.object(
        job_manager.driver, "page_source", return_value="some job content"
    )
    ...
    mocker.patch.object(
        job_manager.driver, "find_elements", return_value=[container_mock]
    )
    ...
    mocker.patch.object(
        ...
        return_value=("Title", "Company", "Location", "Apply", "Link"),
    )
